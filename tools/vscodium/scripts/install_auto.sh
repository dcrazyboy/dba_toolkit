...
if [ "$1" = "--dry-run" ]; then
  ...
  echo "   - Le script vérifiera les prérequis (disque, distro, etc.) comme en mode réel."
  echo "     et s'arrêtera en cas d'erreur, comme une exécution normale."
  echo "   - Hormis les prérequis, aucune modification ne sera appliquée."
fi
# =============================================
...
if [ "$DRY_RUN" = false ]; then
  ...
  echo -e "\n[DRY RUN] 📌 [Étape 2/5] Vérification du disque externe..."
fi
...
if [ ! -d "$RACINE_EXT" ]; then
  if [ "$DRY_RUN" = false ]; then
    ...
    echo "[DRY RUN] ⚠️ Erreur : Le disque externe (${RACINE_EXT}) n'est pas monté ou inaccessible."
    echo "[DRY RUN]  - Branchez votre disque externe et réessayez."
    echo "[DRY RUN]  - Sous WSL, utilisez /mnt/<lettre> pour accéder aux disques Windows."
  fi
  exit 1
fi
if [ ! -w "$RACINE_EXT" ]; then
  if [ "$DRY_RUN" = false ]; then
    ...
    echo "[DRY RUN] ⚠️ Erreur : Pas de permissions en écriture sur ${RACINE_EXT}."
    echo "[DRY RUN]  - Utilisez 'sudo chmod' ou 'sudo chown' pour ajuster les permissions."
  fi
  exit 1
fi
if [ "$DRY_RUN" = false ]; then
...
if ! command -v codium &> /dev/null; then
	...
	if [ "$DRY_RUN" = false ]; then
    echo "Une installation deVSCodium est détecté. O, passe a l'étape suivante."
  else
    echo "[DRY RUN] Une installation deVSCodium est détectée. O, passe a l'étape suivante."
	fi
fi
...
if [ "$DRY_RUN" = false ]; then
  if ! command -v codium &> /dev/null; then
    echo "⚠️ Erreur : VSCodium n'est pas installé. Impossible d'installer les extensions."
    exit 1
  fi
  ...
  if ! command -v codium &> /dev/null; then
    echo "[DRY RUN] VSCodium n'est pas installé. Simulation d'installation des extensions non significative."
  else
    echo "[DRY RUN] Une installation deVSCodium est détectée. Simulation d'installation des extensions non significative."
  fi
  echo "   [DRY RUN] Commandes pour installer les extensions (simulation) :"
...
if [ "$DRY_RUN" = false ]; then
  ...
  mkdir -p "${FULL_PATH_EXT}/{${REPO_PRIV},${REPO_COL},${REPO_PUB}}" || { echo "⚠️ Échec : Création des dossiers externes."; exit 1; }
