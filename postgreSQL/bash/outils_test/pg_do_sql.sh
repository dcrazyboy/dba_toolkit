#!/bin/bash
#------
# Script d'execution en masse d'instruction sql (ou pas) base sur le resultat du discoverpg
# author : JCN
#------
# Variables globales
PROGNAME=$(basename "$0")
...
while true; do
    read -p "Entrez le nom du script, dans ${PATHTOOLS} $ : " set_script
    case "$set_out" in
        *) do_verif_file "${PATHTOOLS}/${set_script}" "force"
            if [[ $? == $ST_OK ]];then 
              break
            fi
           ;;
