#!/bin/bash
#------
# Script d'affichage de la structure des roles/useraprès sélection de la cible
# author : JCN
#------
# Variables globales
PROGNAME=$(basename "$0")
...
command="WITH RECURSIVE role_hierarchy AS (
    -- On part des rôles qui n'ont pas de parent (ancêtres)
    SELECT
        r.oid AS child_oid,
        r.rolname AS child_role,
        NULL::oid AS parent_oid,
        NULL::name AS parent_role,
        0 AS level,
        ARRAY[r.rolname] AS role_path  -- Chemin sous forme de noms de rôles
    FROM pg_roles r
    WHERE NOT EXISTS (
    ...
    -- On descend vers les enfants
    SELECT
        m.member AS child_oid,
        cr.rolname AS child_role,
        rh.child_oid AS parent_oid,
        rh.child_role AS parent_role,
        rh.level + 1,
        rh.role_path || cr.rolname  -- Ajoute le rôle enfant au chemin
    FROM role_hierarchy rh
    JOIN pg_auth_members m ON rh.child_oid = m.roleid
    JOIN pg_roles cr ON m.member = cr.oid
    WHERE NOT m.member = ANY(
        SELECT unnest(ARRAY(SELECT oid FROM pg_roles WHERE rolname = ANY(rh.role_path)))
    )  -- Évite les cycles
),

-- Formattage du chemin complet
formatted_hierarchy AS (
    SELECT
        child_role,
        parent_role AS enfant_direct,  -- Renommé pour plus de clarté
        level,
        role_path,
        array_to_string(role_path, ' => ') AS chemin_complet
    FROM role_hierarchy
    WHERE child_role IS NOT NULL
)
SELECT
    repeat('    ', level) || child_role AS \"Hiérarchie des rôles\",
    enfant_direct AS \"Enfant direct\",
    level AS \"Niveau\",
    chemin_complet AS \"Chemin complet (ancêtre => ... => enfant)\"
FROM formatted_hierarchy
ORDER BY role_path;"
...
if [[ ${lsel[*]:-vide} == "vide" ]];then
  ...
  for i in "${!lsel[@]}"; do
    ...
    if [[ "${set_out}" == "E" || "${set_out}" == "e" ]];then
      ...
      fic_out="${PATHOUTPUT}/check_user_${db_name}_${db_env}.log"
      if [ -f "${fic_out}" ];then
        rm "${fic_out}" 2>"${PATHDATAS}/error.log"
      fi
      do_db_command "${db_host}" "${db_port}" "${db_name}" "${pg_user}" "${mode}" "${command}" > "${fic_out}"
